...
import static org.sRandomRTP.DifferentMethods.rtprtps.ComputeSessionSeed.computeSessionSeed;
...
public class RtpRtpFar {
    public static void rtpRtpfar(CommandSender sender, World targetWorld) {
        ...
        try {
            ...
            int radius = getWorldSpecificFarRadius(world, "radius-far");
            int minRadius = getWorldSpecificFarRadius(world, "minradius-far");
            ...
            if (radius > maxPossibleRadius) {
                if (loggingEnabled) {
                    Bukkit.getConsoleSender().sendMessage("[sRandomRTP-Far] Radius " + radius + " exceeds world border size (" + (int)borderSize + "). Adjusting to " + maxPossibleRadius);
                }
                radius = maxPossibleRadius;
                radiusAdjusted = true;
            }

            if (minRadius > maxPossibleRadius) {
                minRadius = Math.max(0, maxPossibleRadius - 100);
                radiusAdjusted = true;
            }

            if (minRadius >= radius) {
                minRadius = Math.max(0, radius - 100);
                radiusAdjusted = true;
            }

            if (radiusAdjusted && loggingEnabled) {
                Bukkit.getConsoleSender().sendMessage("[sRandomRTP-Far] Final adjusted values: radius=" + radius + ", minRadius=" + minRadius);
            }
            ...
            searchStarted = true;
            long sessionNonce = computeSessionSeed(player, context);

            Variables.playerSearchStatus.put(player.getName(), true);

            scheduleNextAttempt(player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
                    config, context, sessionNonce, 0L);
        } catch (Throwable e) {
    ...
    private static void scheduleNextAttempt(Player player, World world, int centerX, int centerZ, int radius, int minRadius,
                                            int maxAttempts, boolean loggingEnabled, FileConfiguration config,
                                            TeleportRequestContext context, long sessionNonce, long delayTicks) {
        if (!player.isOnline()) {
            if (loggingEnabled) {
        ...
        WrappedTask task = Variables.getFoliaLib().getImpl().runLater(() ->
                        attemptCoordinate(player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
                                config, context, sessionNonce),
                Math.max(delayTicks, 0L));
        TeleportRequestManager.registerTask(player, task);
    }

    private static void attemptCoordinate(Player player, World world, int centerX, int centerZ, int radius, int minRadius,
                                          int maxAttempts, boolean loggingEnabled, FileConfiguration config,
        ...
        if (radius == minRadius) {
            List<String> formattedMessage = LoadMessages.error_radius;
        ...
        Runnable retryCallback = () -> scheduleNextAttempt(player, world, centerX, centerZ, radius, minRadius,
                maxAttempts, loggingEnabled, config, context, sessionNonce, 1L);
        ...
        String generationMethod = Variables.farfile.getString("teleport-far.coordinate-generation-far");
        boolean useAbsoluteCoordinates = Variables.farfile.getBoolean("teleport-far.use-absolute-coordinates-far");

        boolean useParallelSearch = shouldUseParallelCandidateSearch(player, world, config, context);
        int configuredBatchSize = Math.max(1, Variables.teleportfile.getInt("teleport.parallel-search.candidates-per-batch", 1));
        int batchSize = 1;
        if (useParallelSearch) {
            batchSize = Math.min(Math.max(2, configuredBatchSize), 3);
        }
        ...
        for (int i = 0; i < batchSize; i++) {
            ...
            CandidateSearchTask task = createCandidateSearchTask(player, world, centerX, centerZ, radius, minRadius,
                    sessionNonce, generationMethod, useAbsoluteCoordinates, loggingEnabled, attemptNumber,
                    generationIndex, i + 1, batchSize, context);
            if (task != null) {
        ...
        for (CandidateSearchTask task : candidateTasks) {
            task.future.whenComplete((candidate, throwable) -> {
                ...
                boolean success = throwable == null && candidate != null && candidate.y != -1;
                if (success) {
                    if (winnerChosen.compareAndSet(false, true)) {
                        ...
                        Variables.getFoliaLib().getImpl().runAtLocation(processingLocation, processingTask -> {
                            ...
                            processCandidateLocation(player, world, centerX, centerZ, task.x, task.z, candidate, loggingEnabled, config, attemptNumber, maxAttempts, retryCallback, context);
                        });
                    }
                    return;
                }

                if (throwable == null && candidate != null && candidate.y == -1 && loggingEnabled && shouldLogAttempt(attemptNumber)) {
                    Bukkit.getConsoleSender().sendMessage("Teleportation attempt #" + attemptNumber + " failed due to unsafe location.");
                }

                if (remaining.decrementAndGet() == 0 && !winnerChosen.get()) {
                    if (attemptResolved.compareAndSet(false, true)) {
                        attemptFuture.complete(null);
                        handleFailedAttempt(player, loggingEnabled, attemptNumber, maxAttempts, retryCallback, context);
                    } else {
    ...
    private static CandidateSearchTask createCandidateSearchTask(Player player, World world, int centerX, int centerZ, int radius,
                                                                 int minRadius, long sessionNonce, String generationMethod,
                                                                 boolean useAbsoluteCoordinates, boolean loggingEnabled,
                                                                 int attemptNumber, int generationIndex, int candidateNumber,
                                                                 int batchSize, TeleportRequestContext context) {
        if (context.isCancelled() || context.isCompleted()) {
            return new CandidateSearchTask(centerX, centerZ, CompletableFuture.completedFuture(null));
        }

        int normalizedIndex = Math.max(0, generationIndex);
        int[] generatedCoordinates = GenerateCoordinates.generateCoordinates(player.getName(), normalizedIndex, sessionNonce,
                centerX, centerZ, radius, minRadius, generationMethod, useAbsoluteCoordinates);
        int finalNewX = generatedCoordinates[0];
        int finalNewZ = generatedCoordinates[1];

        if (loggingEnabled && shouldLogAttempt(attemptNumber)) {
            double distance = Math.sqrt(Math.pow(finalNewX - centerX, 2) + Math.pow(finalNewZ - centerZ, 2));
            if (batchSize > 1) {
                Bukkit.getConsoleSender().sendMessage("Generated coordinates (candidate " + candidateNumber + "/" + batchSize
                        + "): X=" + finalNewX + ", Z=" + finalNewZ + ", Distance from center: " + (int) distance + " blocks");
            } else {
                Bukkit.getConsoleSender().sendMessage("Generated coordinates: X=" + finalNewX + ", Z=" + finalNewZ
                        + ", Distance from center: " + (int) distance + " blocks");
            }
        }
        ...
        if (border != null && !border.isInside(new Location(world, finalNewX + 0.5, player.getLocation().getY(), finalNewZ + 0.5))) {
            if (loggingEnabled && shouldLogAttempt(attemptNumber)) {
    ...
    private static boolean shouldUseParallelCandidateSearch(Player player, World world, FileConfiguration config,
                                                            TeleportRequestContext context) {
        if (Variables.teleportfile == null) {
            return false;
        }
        if (!Variables.teleportfile.getBoolean("teleport.parallel-search.enabled", false)) {
            return false;
        }
        int batchSize = Variables.teleportfile.getInt("teleport.parallel-search.candidates-per-batch", 1);
        return batchSize >= 2;
    }

    private static void processCandidateLocation(Player player, World world, int centerX, int centerZ, int finalNewX, int finalNewZ,
                                                 CoordinateCandidate candidate, boolean loggingEnabled, FileConfiguration config, int attemptNumber, int maxAttempts, Runnable retryCallback,
                                                 TeleportRequestContext context) {
        ...
        if (safeSurface && newY != -1) {
            ...
            chunkLoadFuture.thenAccept(timedOut -> {
                ...
                if (loggingEnabled) {
                    ...
                    ex.printStackTrace();
                    Bukkit.getLogger().info("Trying direct teleportation despite chunk loading error");
                }
                if (player.isOnline()) {
                    if (loggingEnabled) {
                        Bukkit.getLogger().info("Directly teleporting player " + player.getName() + " to location " + teleportLocation);
                    }
                    TeleportWithChunkLoading.teleportWithChunkLoading(player, teleportLocation, loggingEnabled, finalNewX, finalNewZ, newY);
                    TeleportRequestManager.completeRequest(context, loggingEnabled);
                    Variables.playerSearchStatus.put(player.getName(), false);
    ...
    private static int getWorldSpecificFarRadius(World world, String setting) {
        String worldName = world.getName();
        String worldPath = "teleport-far.per-world-far." + worldName + "." + setting;
        if (Variables.farfile.contains(worldPath)) {
            return Variables.farfile.getInt(worldPath);
        }
        return Variables.farfile.getInt("teleport-far." + setting);
    }

    private static class CandidateSearchTask {
        final int x;
        final int z;
