...
public class RtpRtpPlayer {
    public static void rtprtpplayer(CommandSender sender, Player player, World targetWorld) {
        ...
        try {
            ...
            if (world == null) {
                sender.sendMessage(pluginName + " §8- §cUnable to determine a valid world for teleportation.");
                return;
            }
            ...
            scheduleNextAttempt(sender, player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
    ...
    private static void scheduleNextAttempt(CommandSender sender, Player player, World world, int centerX, int centerZ, int radius, int minRadius,
        ...
        WrappedTask task = Variables.getFoliaLib().getImpl().runLater(() ->
                        attemptCoordinate(sender, player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
                                config, context, sessionNonce),
                Math.max(delayTicks, 0L));
        TeleportRequestManager.registerTask(player, task);
    }

    private static void attemptCoordinate(CommandSender sender, Player player, World world, int centerX, int centerZ, int radius, int minRadius,
                                          int maxAttempts, boolean loggingEnabled, FileConfiguration config,
        ...
        Runnable retryCallback = () -> scheduleNextAttempt(sender, player, world, centerX, centerZ, radius, minRadius,
                ...
                if (success) {
                    if (winnerChosen.compareAndSet(false, true)) {
                        ...
                        Variables.getFoliaLib().getImpl().runAtLocation(processingLocation, processingTask -> {
                            ...
                            processCandidateLocation(sender, player, world, centerX, centerZ, task.x, task.z, candidate, loggingEnabled, config, attemptNumber, maxAttempts, retryCallback, context);
                        });
                    }
                    return;
                }
    ...
    private static void processCandidateLocation(CommandSender sender, Player player, World world, int centerX, int centerZ, int finalNewX, int finalNewZ,
        ...
        if (safeSurface && newY != -1) {
            ...
            chunkLoadFuture.thenAccept(timedOut -> {
                ...
                // Send success messages - специфика RtpRtpPlayer
                ...
                if (originalSender != null) {
                    ...
                    List<String> formattedMessage1 = LoadMessages.rtpplayerteleportsuccesssender;
