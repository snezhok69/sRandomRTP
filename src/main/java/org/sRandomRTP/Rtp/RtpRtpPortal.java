...
public class RtpRtpPortal {
    public static void rtprtpportal(CommandSender sender, Player player, World targetWorld) {
        ...
        try {
            if (player == null) {
                sender.sendMessage(pluginName + " §8- §cPlayer cannot be null.");
                return;
            }
            ...
            if (radius > maxPossibleRadius) {
                if (loggingEnabled) {
                    Bukkit.getConsoleSender().sendMessage("[sRandomRTP-Portal] Radius " + radius + " exceeds world border size (" + (int)borderSize + "). Adjusting to " + maxPossibleRadius);
                }
            ...
            if (radiusAdjusted && loggingEnabled) {
                Bukkit.getConsoleSender().sendMessage("[sRandomRTP-Portal] Final adjusted values: radius=" + radius + ", minRadius=" + minRadius);
            }
