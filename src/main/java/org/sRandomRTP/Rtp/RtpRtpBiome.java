...
public class RtpRtpBiome {
    public static void rtpRtpBiome(CommandSender sender, World targetWorld, List<Biome> preferredBiomes) {
        ...
        try {
            ...
            List<Biome> biomeTargets = preferredBiomes == null ? java.util.Collections.emptyList() : new ArrayList<>(preferredBiomes);
            ...
            int maxPossibleRadius = (int) Math.floor(borderSize * 0.95); // 95% of border to add safety margin

            boolean radiusAdjusted = false;

            if (radius > maxPossibleRadius) {
                if (loggingEnabled) {
                    Bukkit.getConsoleSender().sendMessage("[sRandomRTP] Radius " + radius + " exceeds world border size (" + (int)borderSize + "). Adjusting to " + maxPossibleRadius);
                }
            ...
            if (radiusAdjusted && loggingEnabled) {
                Bukkit.getConsoleSender().sendMessage("[sRandomRTP] Final adjusted values for world '" + world.getName() + "': radius=" + radius + ", minRadius=" + minRadius);
            }

            if (loggingEnabled) {
                Bukkit.getConsoleSender().sendMessage("[sRandomRTP] radius=" + radius + ", minRadius=" + minRadius);
            }
            ...
            scheduleNextAttempt(player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
                    config, context, sessionNonce, 0L, biomeTargets);
        } catch (Throwable e) {
    ...
    private static void scheduleNextAttempt(Player player, World world, int centerX, int centerZ, int radius, int minRadius,
        ...
        WrappedTask task = Variables.getFoliaLib().getImpl().runLater(() ->
                        attemptCoordinate(player, world, centerX, centerZ, radius, minRadius, maxAttempts, loggingEnabled,
                                config, context, sessionNonce, biomeTargets),
    ...
    private static void attemptCoordinate(Player player, World world, int centerX, int centerZ, int radius, int minRadius,
                                          int maxAttempts, boolean loggingEnabled, FileConfiguration config,
                                          TeleportRequestContext context, long sessionNonce, List<Biome> biomeTargets) {
        if (!player.isOnline()) {
            TeleportRequestManager.cancelRequest(player.getUniqueId(), loggingEnabled, "player offline");
            Variables.playerSearchStatus.put(player.getName(), false);
            return;
        ...
        Runnable retryCallback = () -> scheduleNextAttempt(player, world, centerX, centerZ, radius, minRadius,
                maxAttempts, loggingEnabled, config, context, sessionNonce, 1L, biomeTargets);
        ...
        boolean useAbsoluteCoordinates = Variables.teleportfile.getBoolean("teleport.use-absolute-coordinates");
                ...
                if (success) {
                    if (winnerChosen.compareAndSet(false, true)) {
                        ...
                        Variables.getFoliaLib().getImpl().runAtLocation(processingLocation, processingTask -> {
                            ...
                            processCandidateLocation(player, world, centerX, centerZ, task.x, task.z, candidate, loggingEnabled, config, attemptNumber, maxAttempts, retryCallback, context, biomeTargets);
                        });
                    }
                    return;
                }
