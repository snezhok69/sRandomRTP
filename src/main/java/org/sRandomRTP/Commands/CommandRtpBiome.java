...
import org.sRandomRTP.Cooldowns.CooldownCommandRtpBiome;
...
public class CommandRtpBiome {
    public static void commandRtpBiome(CommandSender sender, String[] biomeArgs) {
        try {
            ...
            if (requestedBiomes == null || requestedBiomes.isEmpty()) {
                return;
            }
            ...
            if (!player.hasPermission("sRandomRTP.Command.RtpBiome")) {
            ...
            //
            if (CooldownCommandRtpBiome.cooldownCommandRtpBiome(player, sender)) {
                return;
            }
            //
            if (moneyEnabled) {
            ...
            //
            if (CooldownBypassBossBarBiome.cooldownBypassBossBarBiome(player, sender, world, requestedBiomes)) {
                return;
            }
            return;
        } catch (Throwable e) {
    ...
    private static List<Biome> parseBiomes(CommandSender sender, String[] biomeArgs) {
        ...
        if (tokens.isEmpty() || tokens.size() > 2) {
            sender.sendMessage(Variables.pluginName + " §cSpecify one or two biomes at most.");
            return null;
        }
        ...
        for (String token : tokens) {
            ...
            if (IsBiomeBanned.isBiomeBanned(biome)) {
