...
public class GetSafeYCoordinateInNether {
    public static CompletableFuture<Integer> getSafeYCoordinateInNetherAsync(World world, int x, int z) {
        return getSafeYCoordinateInNetherAsync(world, x, z, null);
    }
    ...
    public static CompletableFuture<Integer> getSafeYCoordinateInNetherAsync(World world, int x, int z,
            ...
            RegionTaskExecutor.runAtLocation(location, () -> {
                try {
                    ...
                    try {
                        ...
                        LoggerUtility.loggerUtility(GetSafeYCoordinateInNether.class.getName(), t);
                        future.complete(-1);
                        return;
                    }
                    ...
                    LoggerUtility.loggerUtility(GetSafeYCoordinateInNether.class.getName(), e);
                    future.complete(-1);
                }
            });
            return future;
        ...
        AsyncChunkUtil.requestChunk(world, x >> 4, z >> 4).whenComplete((chunk, throwable) -> {
            if (throwable != null) {
                LoggerUtility.loggerUtility(GetSafeYCoordinateInNether.class.getName(), throwable);
                future.complete(-1);
                return;
            }
    ...
    private static int binarySearchSafeY(World world, int x, int z, int minY, int maxY, TeleportRequestContext context, boolean loggingEnabled) {
        if (isSafePosition(world, x, maxY, z)) {
            if (loggingEnabled) {
                Variables.instance.getLogger().info("[Nether] Safe Y-coordinate found at top: " + maxY);
            }
            return maxY;
        }
        ...
        while (low <= high) {
            ...
            if (isSafePosition(world, x, mid, z)) {
                if (loggingEnabled) {
                    Variables.instance.getLogger().info("[Nether] Safe Y-coordinate found: " + mid);
                }
                return mid;
            }
    ...
    private static boolean isSafePosition(World world, int x, int y, int z) {
        int configMinY = Variables.teleportfile.getInt("teleport.minY-nether");
        if (y < configMinY) {
            return false;
        }
        ...
        return belowBlock.getType().isSolid() &&
                !belowBlock.getType().name().contains("BEDROCK") &&
                twoBelowBlock.getType().isSolid() &&
                !twoBelowBlock.getType().name().contains("BEDROCK");
    }
    ...
    private static int linearSearchFromTop(World world, int x, int z, int minY, int maxY, TeleportRequestContext context, boolean loggingEnabled) {
        for (int y = maxY; y > minY; y--) {
            ...
            if (isSafePosition(world, x, y, z)) {
                if (loggingEnabled) {
                    Variables.instance.getLogger().info("[Nether] Safe Y-coordinate found with linear search: " + y);
                }
                return y;
            }
        }
        ...
        if (loggingEnabled) {
            Variables.instance.getLogger().warning("[Nether] Failed to find a safe Y-coordinate at the point X:" + x + ", Z:" + z);
        }
        return -1;
    }
    ...
    private static boolean shouldAbort(TeleportRequestContext context, boolean loggingEnabled) {
        if (context != null && (context.isCancelled() || context.isExpired())) {
            if (loggingEnabled) {
                Variables.instance.getLogger().fine("[Nether] Aborting height search due to request cancellation");
            }
            return true;
        }
        return false;
