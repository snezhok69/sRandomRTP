...
public class CooldownBypassBossBarBiome {
    ...
    public static boolean cooldownBypassBossBarBiome(Player player, CommandSender sender, World world, List<Biome> targetBiomes) {
        try {
            List<Biome> immutableBiomes = targetBiomes == null
                    ? Collections.emptyList()
                    : Collections.unmodifiableList(new ArrayList<>(targetBiomes));
            if (player.hasPermission("sRandomRTP.Cooldown.bypass")) {
                RtpRtpBiome.rtpRtpBiome(sender, world, immutableBiomes);
            } else {
                startBossBarCountdown(player, sender, world, immutableBiomes);
                return true;
            }
        } catch (Throwable e) {
    ...
    public static void startBossBarCountdown(Player player, CommandSender sender, World world, List<Biome> targetBiomes) {
        ...
        WrappedTask progressTask = Variables.getFoliaLib().getImpl().runTimerAsync(() -> {
            ...
            if (timeLeft[0] <= 0) {
                ...
                RtpRtpBiome.rtpRtpBiome(sender, world, targetBiomes);
